...
Normalisation note
------------------
All functions return raw float32 pixels in [0, 255].  The conversion to the
MobileNetV2 expected range [-1, 1] is performed *inside the model* by the
``mobilenetv2_preprocess`` Lambda layer baked into the Keras architecture.
This guarantees identical treatment during training and inference.
"""
...
IMG_SIZE = (224, 224)
PIXEL_RANGE = (0.0, 255.0)  # raw float32; model bakes [-1, 1] scaling internally
...
def load_and_preprocess(image_path: str | Path) -> np.ndarray:
    ...
    4. Convert to numpy float32 array — values in [0, 255]

    The [-1, 1] scaling required by MobileNetV2 is applied inside the model.

    Returns: numpy array shape (224, 224, 3), dtype float32, values in [0, 255]
    """
...
def preprocess_image(image_input: str | Path | Image.Image) -> np.ndarray:
    ...
    Converts to RGB, resizes to 224x224 as float32 in [0, 255].
    The [-1, 1] scaling for MobileNetV2 is applied inside the model.
    ...
    Returns:
        numpy array of shape (224, 224, 3), dtype float32, values in [0, 255].
    """
