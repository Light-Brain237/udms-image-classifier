"""
illegal_dumping_scraper.py
Category-targeted image scraper with three-layer filtering.

Category: illegal_dumping / garbage
...
Run:
  python illegal_dumping_scraper.py
"""
...
# ============================================================
# PLACES — same African city/country grid as pothole scraper
# ============================================================
...
PLACES = [
    ...
    ("Brazzaville", "fr"), ("Niamey", "fr"), ("Bangui", "fr"),
    # Anglophone
    ("Lagos", "en"), ("Abuja", "en"), ("Kano", "en"), ("Ibadan", "en"),
    ("Benin City", "en"), ("Port Harcourt", "en"), ("Aba", "en"),
    ("Accra", "en"), ("Kumasi", "en"), ("Takoradi", "en"),
    ("Nairobi", "en"), ("Mombasa", "en"), ("Kisumu", "en"),
    ("Kampala", "en"), ("Dar es Salaam", "en"), ("Mwanza", "en"),
    ("Johannesburg", "en"), ("Cape Town", "en"), ("Durban", "en"),
    ("Harare", "en"), ("Lusaka", "en"), ("Kigali", "en"),
    ("Addis Ababa", "en"), ("Freetown", "en"), ("Monrovia", "en"),
    ("Accra", "en"), ("Bamenda", "en"),
    ("Nigeria", "en"), ("Ghana", "en"), ("Kenya", "en"),
    ("Uganda", "en"), ("Tanzania", "en"), ("Cameroon", "en"),
    # Lusophone
    ("Luanda", "pt"), ("Maputo", "pt"), ("Beira", "pt"), ("Bissau", "pt"),
    ("Nacala", "pt"),
    # Swahili belt (EN queries, Swahili keywords handled in GENERIC_EXTRAS)
    ("Dar es Salaam", "en"), ("Nairobi", "en"),
]

MODIFIERS = {
    "en": [
        "illegal dumping", "illegal garbage dump", "fly tipping",
        "open waste dump", "rubbish dumped on street",
        "trash pile street", "garbage heap urban",
        "waste dumped roadside", "littering urban street",
        "open dump site", "refuse pile neighborhood",
    ],
    "fr": [
        "dépôt sauvage", "décharge sauvage", "ordures rue",
        "déchets abandonnés", "tas d'ordures", "détritus rue",
        "décharge illégale", "poubelles renversées rue",
        "déchets ménagers abandonnés",
    ],
    "pt": [
        "lixo na rua", "despejo ilegal lixo", "lixo abandonado",
        "entulho rua", "descarte ilegal lixo", "monte lixo urbano",
    ],
}

GENERIC_EXTRAS = [
    # English generic
    "illegal dumping Africa street",
    "fly tipping Africa",
    "open garbage dump Africa",
    "waste pile African city",
    "trash heap urban Africa",
    "illegal waste disposal Africa",
    "garbage dumped roadside Africa",
    "open dump site sub-saharan Africa",
    "refuse pile African neighborhood",
    "household waste dumped street Africa",
    "market waste pile Africa",
    "plastic waste pile Africa street",
    # Social media (higher image-caption match)
    "illegal dumping site:reddit.com",
    "fly tipping site:reddit.com",
    "illegal dumping Africa site:flickr.com",
    "garbage dump site:twitter.com Nigeria",
    "dépôt sauvage site:twitter.com",
    "illegal dump Nairobi",
    "illegal dump Lagos",
    "illegal dump Accra",
    "illegal dump Kampala",
    "illegal dump Kinshasa",
    "illegal dump Douala",
    "illegal dump Johannesburg",
    "garbage dump Nairobi street",
    "garbage dump Lagos street",
    # Swahili
    "taka mtaani",
    "utupaji taka haramu",
    "takataka mtaani Nairobi",
    "takataka barabarani",
]
...
# ============================================================
# CLIP PROMPTS — tuned for illegal dumping / garbage
# ============================================================

KEEP_PROMPTS = [
    "a photograph of an illegal garbage dump on a street",
    "a photograph of trash and rubbish piled on a roadside",
    "a photograph of fly tipping — waste illegally dumped in a public place",
    "a photograph of a large heap of garbage in an urban neighborhood",
    "a photograph of plastic waste and refuse bags dumped on the ground",
    "a photograph of an open waste dump site in an African city",
    "a photograph of scattered litter and debris on a street",
    "a photograph of household waste dumped next to a road",
    "a photograph of a pile of waste bags and rubbish on a sidewalk",
    "a photograph of an informal garbage dump in a tropical city",
]

REJECT_PROMPTS = [
    "a portrait photograph of a person",
    "a photograph of a politician at a podium giving a speech",
    "a photograph of a clean and tidy street with no rubbish",
    "a photograph of a modern waste recycling facility or sorting plant",
    ...
    "a photograph of a garbage truck collecting bins",
    "a photograph of neatly placed rubbish bins on a pavement",
    "a photograph of a landfill seen from the air",
    "a photograph of a pothole or damaged road surface",
    "a photograph of bridge or building construction",
    "a snowy outdoor scene",
]

# Soft Africa relevance ranking (same logic as pothole scraper)
AFRICA_PROMPTS = [
    ...
    "a dusty urban street in tropical Africa",
]
NON_AFRICA_PROMPTS = [
...
SUSPICIOUS_IMG_TOKENS = [
    ...
    "recycling-plant", "recycling_plant", "landfill-aerial", "landfill_aerial",
    # stock / watermark
...
ALT_STRONG = [
    # EN
    "illegal dump", "illegal dumping", "fly tip", "fly tipping",
    "garbage dump", "rubbish dump", "waste dump", "open dump",
    "trash heap", "rubbish heap", "garbage pile", "waste pile",
    "litter", "littering", "refuse", "dumped waste", "dumped rubbish",
    # FR
    "dépôt sauvage", "décharge sauvage", "ordures", "déchets",
    "tas d'ordures", "détritus",
    # PT
    "lixo", "entulho", "despejo ilegal", "lixo abandonado",
    # SW
    "taka", "takataka", "utupaji taka",
]

ALT_ARTICLE_ONLY = [
    "budget", "billion", "parliament", "government", "minister",
    "announce", "speech", "statement", "report", "policy",
    "recycling target", "waste management plan",
]
...
KEEP_MARGIN = 1.05       # keep score must exceed reject score by this ratio
MIN_KEEP_SCORE = 0.18    # absolute CLIP floor

# Higher PER_QUERY than pothole scraper to reach ~20k raw target
# ~170 unique queries × 120 per engine × 3 engines ≈ 61,200 attempts
PER_QUERY = 120
MIN_W, MIN_H = 400, 300
...
# Saved inside the project under data/raw/illegal_dumping_scraped/
OUT_DIR    = Path(__file__).parent / "data" / "raw" / "illegal_dumping_scraped"
...
def main():
    ...
    print(f"    PER_QUERY: {PER_QUERY} × ~3 engines = ~{len(QUERIES) * PER_QUERY * 3:,} attempts\n")
    ...
    print("\nNext steps:")
    print(f"  1. Review {KEEP_DIR} — 'a_' prefixed = highest African-context relevance.")
    print(f"  2. Spot-check {REJECT_DIR} — real dumps missed? lower MIN_KEEP_SCORE.")
    print(f"  3. Move verified keeps → data/processed/train/illegal_dumping/")
